    ...
    def createRewards(self):
        for state in self.states:
            ...
            for action in self.actions:
                ...
                if state == (Grid.WIDTH - 1, Grid.HEIGHT - 1): self.rewards[state][action] = Grid.END_REWARD
                if state == (Grid.WIDTH - 1, Grid.HEIGHT - 2): self.rewards[state][action] = -Grid.END_REWARD
