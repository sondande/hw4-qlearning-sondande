...
/**
 * The Grid class defines the Grid that the agent operates in.
...
public class Grid {
    ...
    /** The final reward for reaching the goal. */
    private static final double END_REWARD = 5.0;
        ...
        for (State state : states) {
            ...
            for (String action : actions) {
                if (state.x == Grid.WIDTH - 1 && state.y == Grid.HEIGHT - 1) {
                    rewards.get(state).put(action, END_REWARD);
                } else if (state.x == Grid.WIDTH - 1 && state.y == Grid.HEIGHT - 2) {
                    rewards.get(state).put(action, -1.0 * END_REWARD);
